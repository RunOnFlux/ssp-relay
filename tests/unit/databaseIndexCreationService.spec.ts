...
describe('Database Index Creation Service', () => {
  describe('Do Index: Correctly verifies index', () => {
    before(async () => {
      const db = await serviceHelper.databaseConnection();
      database = db.db(config.database.database);
    })

    it('should return when collection v1action index initialized', async () => {
      ...
      await serviceHelper.findOneInDatabase(
        database,
        v1action,
        { wkIdentity: 1 },
        projection,
    ...
    it('should return when collection v1token index initialized', async () => {
