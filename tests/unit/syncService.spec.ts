...
let syncCollection = undefined;
let tokenCollection = undefined;

describe('Sync Service', function () {
  describe('Get Sync: Correctly verifies get sync', function () {
    ...
    it('should return error when id is undefined', async function () {
      await syncService
        .getSync()
        .catch((e) => assert.equal(e, 'Error: Sync undefined not found'));
    });

    it('should return error when id is not valid', async function () {
      await syncService
        .getSync(123)
        .catch((e) => assert.equal(e, 'Error: Sync 123 not found'));
    });
    ...
    it('should return error after database drop and id is invalid', async function () {
      ...
      await syncService
        .getSync(141)
        .catch((e) => assert.equal(e, 'Error: Sync 141 not found'));
    });
  ...
  describe('Post Sync: Correctly verifies post sync', function () {
    it('should return data with walletIdentity when id is valid', async function () {
    ...
    it('should return data without walletIdentity when data is empty', async function () {
    ...
    it('should return data without walletIdentity when id is invalid', async function () {
      await syncService.postSync().catch((r) => {
        assert.equal(
          r,
          `TypeError: Cannot read properties of undefined (reading 'walletIdentity')`,
        );
      });
    });
  });

  describe('Get tokens: Correctly verifies get tokens', function () {
    before(async function () {
      const db = await serviceHelper.databaseConnection();
      database = db.db(config.database.database);
    ...
    it('should return successful result 141 if stub value is valid', async function () {
      await sinon
        .stub(serviceHelper, 'findInDatabase')
    ...
    it('should return error result if stub value is empty', async function () {
  ...
  describe('Post Token: Correctly verifies post token', function () {
    afterEach(function () {
      sinon.restore();
    });
    ...
    it('should return error result 141 if stub value with 100 keys', async function () {
      const call = await sinon.stub(serviceHelper, 'findInDatabase');
      await call.onCall(0).returns([{ wkIdentity: 141 }]);
      await call
        .onCall(1)
      ...
      await syncService
        ...
        .catch((e) =>
          assert.equal(
            e,
            `Error: More than 100 tokens for 141 found, not storing new one`,
          ),
        );
    });
  });
