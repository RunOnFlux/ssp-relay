...
describe('Transaction Decoder', () => {
  describe('Decode Transaction: Correctly verifies action', () => {
    it('should return error result if raw tx is empty', async () => {
      const response = await transactionDecoder.decodeTransactionForApproval({});
      expect(response).to.deep.equal({ amount: "decodingError", receiver: "decodingError" });
    });

    it('should return error result if raw tx is undefined', async () => {
        const response = await transactionDecoder.decodeTransactionForApproval(undefined);
        expect(response).to.deep.equal({ amount: "decodingError", receiver: "decodingError" });
    });

    it('should return error result if raw tx is undefined', async () => {
        const response = await transactionDecoder.decodeTransactionForApproval(null);
        expect(response).to.deep.equal({ amount: "decodingError", receiver: "decodingError" });
    });

    it('should return successful result if raw tx is valid for sepolia', async () => {
        const response = await transactionDecoder.decodeTransactionForApproval(JSON.stringify(rawTxSepolia), "sepolia");
