...
describe('Rates API', () => {
  describe('Get Rates API: Correctly verifies rates', () => {
    afterEach(function() {
      sinon.restore();
    });
    ...
    it('should return successful result if stub value is valid', async () => {
      ...
      expect(JSON.parse(res._getData()).crypto).to.deep.equal({btc:1, flux: 1});
    });

    it('should return Not Found if stub value is undefined', async () => {
        const request = httpMocks.createRequest({
            method: 'GET',
            url: 'test',
          });
    ...
    it('should return Not Found if stub value is false', async () => {
        const request = httpMocks.createRequest({
            method: 'GET',
            url: 'test',
          });
    ...
    it('should return Not Found if stub value is null', async () => {
        const request = httpMocks.createRequest({
            method: 'GET',
            url: 'test',
          });
