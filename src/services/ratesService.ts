...
async function fetchCryptoRates() {
  try {
    ...
    const url = `https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?CMC_PRO_API_KEY=${apiKey}&id=1,3029,2577,2,74,1027,1437,1831,825,3408,3717,2396,1958,11419,5994,1975,3957,4943,7083,3773,24478,26081,28321,5690,3897,3635,10603,7278,11841,27075,7226,1518,6719,11092,10804,23095,8425,10368,6536,8000,3155,16086,21159,4558,4269,19891,28298,6783,22691,13502,5176,7080,6210,13855,2694,1966,29587,9481,2563,4705,4066,7186,18876,1518,27772,7326,30171,26997,9104,3640,9816,17799,8290,1659,4041,5692,8119,10407,23121,27659,21585,12220,3964,2777,29676,6538,8104,18069,7501,6945,14806,23711,2682,1455,28301,32120,2299,1697,3783,1896,29814,21259,12999,27565,21846,8536,2467,2943`;
    ...
    const prices = {
      ...
      kava: response.data.data['4846'].quote.USD.price,
      virtual: response.data.data['29420'].quote.USD.price,
      xcn: response.data.data['18679'].quote.USD.price,
      matic: response.data.data['3890'].quote.USD.price,
      spx: response.data.data['28081'].quote.USD.price,
      ctc: response.data.data['5198'].quote.USD.price,
      morpho: response.data.data['34104'].quote.USD.price,
      eigen: response.data.data['30494'].quote.USD.price,
      ath: response.data.data['30083'].quote.USD.price,
      zkj: response.data.data['29779'].quote.USD.price,
      rose: response.data.data['7653'].quote.USD.price,
      zil: response.data.data['2469'].quote.USD.price,
      ach: response.data.data['6958'].quote.USD.price,
      auction: response.data.data['8602'].quote.USD.price,
      cvx: response.data.data['9903'].quote.USD.price,
      moca: response.data.data['31526'].quote.USD.price,
      red: response.data.data['21707'].quote.USD.price,
    };
    cryptoRates = prices;
  } catch (error) {
    log.error(error);
